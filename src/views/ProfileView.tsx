...
export const ProfileView: React.FC<{ profile: UserProfile; onUpdateProfile?: (profile: UserProfile) => void }> = ({ profile, onUpdateProfile }) => {
  const { t } = useLanguage();
  const { isPremium, upgradeToPremium } = useGamification();
    ...
    <div className="max-w-5xl mx-auto space-y-10 pb-20">
        ...
        <div className="flex-1 text-center md:text-left space-y-4">
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-3">
            <span className={`px-5 py-2 rounded-full text-xs font-black uppercase tracking-widest border ${
              isPremium ? 'bg-amber-100 text-amber-800 border-amber-200' : 'bg-slate-100 text-slate-600 border-slate-200'
            }`}>
              {t('subscriptionStatus')}: {isPremium ? '👑 ' + t('yourStatusPremium') : t('yourStatusFree')}
            </span>
            {!isPremium && (
              <button
                onClick={() => setShowPremiumModal(true)}
                className="px-4 py-2 bg-amber-500 text-white rounded-full text-xs font-black uppercase tracking-widest hover:bg-amber-600 transition"
              >
                Passer Premium
              </button>
            )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm relative overflow-hidden">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{t('dailyObjectives')}</h3>
              {!isEditing && (
                <button onClick={() => setIsEditing(true)} className="text-[10px] font-black text-emerald-600 uppercase tracking-widest hover:underline">
                  Modifier
                </button>
            ...
            <p className="text-xs text-slate-500 mb-6">{t('dailyObjectivesDesc')}</p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { key: 'caloriesGoal', label: t('caloriesGoal'), value: currentProfile.dailyObjectives?.caloriesGoal, unit: 'kcal', placeholder: 2000 },
                { key: 'waterGoalLiters', label: t('waterGoal'), value: currentProfile.dailyObjectives?.waterGoalLiters, unit: 'L', placeholder: 2 },
                { key: 'stepsGoal', label: t('stepsGoal'), value: currentProfile.dailyObjectives?.stepsGoal, unit: '', placeholder: 10000 },
              ].map((m) => (
                <div key={m.key} className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">{m.label}</label>
                  {isEditing ? (
                    <input
                      type="number"
                      placeholder={String(m.placeholder)}
                      value={editProfile.dailyObjectives?.[m.key as keyof NonNullable<typeof editProfile.dailyObjectives>] ?? ''}
                      onChange={(e) => setEditProfile({
                        ...editProfile,
                        dailyObjectives: {
                          ...(editProfile.dailyObjectives ?? {}),
                          [m.key]: e.target.value ? Number(e.target.value) : undefined,
                        },
                      })}
                      className="w-full px-4 py-3 rounded-xl bg-white border-2 border-emerald-300 focus:border-emerald-600 outline-none font-bold text-slate-800"
                    />
                  ) : (
