...
export const SignupView: React.FC<SignupViewProps> = ({ onSignupSuccess, onNavigateToLogin, onBack }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Password validation (OWASP minimum)
    ...
    <div className="min-h-screen bg-[#F9F9F9] overflow-x-hidden text-[#2D3436] flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-white rounded-[3rem] shadow-2xl shadow-slate-900/5 p-12 relative overflow-hidden">
        ...
        <div className="text-center mb-10 pt-4">
          ...
          <h2 className="text-3xl font-black text-[#34495E] tracking-tighter">{t('createAccount')}</h2>
          <p className="text-slate-400 font-medium mt-2">{t('startHealthJourney')}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {isLoading ? t('creatingAccount') : t('continueToProfile')}
          </button>
        </form>
        ...
        <div className="mt-10 text-center">
          <p className="text-slate-400 text-sm font-medium">
            ...
            <button onClick={onNavigateToLogin} className="text-[#2ECC71] font-black hover:underline">{t('signIn')}</button>
          </p>
        </div>
      </div>
    </div>
