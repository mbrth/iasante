      ...
      <div className="bg-white rounded-[3rem] shadow-[0_32px_64px_-12px_rgba(46,204,113,0.15)] w-full max-w-4xl overflow-hidden flex flex-col md:flex-row min-h-[600px] border border-slate-100">
        ...
        <div className="md:w-2/5 bg-brand-secondary p-12 text-white flex flex-col justify-between relative overflow-hidden">
           <div className="relative z-10">
              ...
              <p className="text-slate-200 text-lg font-medium leading-relaxed">{t('onboardingSubtitle')}</p>
           </div>
           ...
           <div className="relative z-10">
              ...
              <p className="mt-4 text-[10px] font-black uppercase tracking-[0.3em] text-brand-primary">{t('stepOf')} {step} {t('of')} 3</p>
           </div>
        ...
        <div className="flex-1 p-12 md:p-20 flex flex-col justify-center">
          {step === 1 && (
            <div className="space-y-10">
              <div>
                <h3 className="text-3xl font-black text-brand-secondary tracking-tight mb-2">{t('theBasics')}</h3>
                <p className="text-slate-400 font-medium">{t('basicsDesc')}</p>
              </div>
              <div className="grid grid-cols-2 gap-8">
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">{t('age')}</label>
                  <input type="number" onChange={e => setProfile({...profile, age: Number(e.target.value)})} className="w-full px-6 py-4 rounded-2xl bg-slate-50 border-2 border-transparent focus:border-brand-primary focus:bg-white transition-all outline-none font-bold text-lg" placeholder={t('years')} />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">{t('biologicalSex')}</label>
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">{t('height')}</label>
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">{t('weight')}</label>
                ...
                <div className="col-span-2">
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">{t('dateOfBirth')}</label>
              ...
              <button onClick={nextStep} className="w-full bg-brand-accent text-white font-black py-5 rounded-[2rem] hover:bg-brand-accent/90 transition-all shadow-xl shadow-brand-accent/20 uppercase tracking-widest text-sm">{t('continue')}</button>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-10">
              <div>
                <h3 className="text-3xl font-black text-brand-secondary tracking-tight mb-2">{t('medicalProfile')}</h3>
                <p className="text-slate-400 font-medium">{t('medicalProfileDesc')}</p>
              </div>
              ...
              <div className="flex gap-4">
                <button onClick={prevStep} className="flex-1 border-2 border-slate-100 text-slate-400 font-bold py-5 rounded-3xl hover:bg-slate-50 transition uppercase tracking-widest text-xs">{t('back')}</button>
                <button onClick={nextStep} className="flex-[2] bg-brand-accent text-white font-black py-5 rounded-3xl hover:bg-brand-accent/90 transition shadow-xl shadow-brand-accent/20 uppercase tracking-widest text-sm">{t('next')}</button>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-10">
              <div>
                <h3 className="text-3xl font-black text-brand-secondary tracking-tight mb-2">{t('dietaryPreferences')}</h3>
                <p className="text-slate-400 font-medium">{t('dietaryDesc')}</p>
              </div>
              <div className="space-y-3">
                {[
                  { key: 'Vegetarian', label: 'vegetarian' },
                  { key: 'Vegan', label: 'vegan' },
                  { key: 'Low-Carb', label: 'lowCarb' },
                  { key: 'Gluten-Free', label: 'glutenFree' },
                  { key: 'Keto', label: 'keto' },
                  { key: 'Mediterranean', label: 'mediterranean' },
                  { key: 'High-Protein', label: 'highProtein' },
                  { key: 'Low-Sodium', label: 'lowSodium' },
                ].map(({ key, label }) => (
                  <button
                    key={key}
                    onClick={() => toggleArrayItem('preferences', key)}
                    className={`w-full text-left px-6 py-4 rounded-2xl text-sm font-bold border-2 transition-all ${
                      profile.preferences?.includes(key) 
              ...
              <div className="space-y-3 pt-4 border-t border-slate-100">
                <h4 className="text-sm font-black text-brand-secondary">{t('primaryGoal')}</h4>
                {[
                  { key: 'Stabilize Blood Glucose', label: 'stabilizeGlucose' },
                  { key: 'Lower Blood Pressure', label: 'lowerBP' },
                  { key: 'Weight Management', label: 'weightManagement' },
                  { key: 'Reduce Cholesterol', label: 'reduceCholesterol' },
                ].map(({ key, label }) => (
                  <button
                    key={key}
              ...
              <div className="flex gap-4 pt-4">
                <button onClick={prevStep} className="flex-1 border-2 border-slate-100 text-slate-400 font-bold py-5 rounded-3xl hover:bg-slate-50 transition uppercase tracking-widest text-xs">{t('back')}</button>
                <button onClick={handleFinish} className="flex-[2] bg-brand-accent text-white font-black py-5 rounded-3xl hover:bg-brand-accent/90 transition shadow-xl shadow-brand-accent/20 uppercase tracking-widest text-sm">{t('initializeSystem')}</button>
              </div>
            </div>
          )}
        </div>
