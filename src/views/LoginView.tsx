    ...
    <div className="min-h-screen bg-[#F9F9F9] overflow-x-hidden text-[#2D3436] flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-white rounded-[3rem] shadow-2xl shadow-slate-900/5 p-12 relative overflow-hidden">
        ...
        <div className="text-center mb-10 pt-4">
          ...
          <h2 className="text-3xl font-black text-[#34495E] tracking-tighter">{t('welcomeBack')}</h2>
          <p className="text-slate-400 font-medium mt-2">{t('accessHealthIntelligence')}</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">{t('emailAddress')}</label>
            <input 
              type="email" 
              required
              value={email}
          ...
          <div className="flex justify-end">
            <a href="#" className="text-xs font-bold text-[#2ECC71] hover:underline">{t('forgotPassword')}</a>
          </div>
          ...
          >
            {isLoading ? t('connecting') : t('signInButton')}
          </button>
        </form>
        ...
        <div className="mt-10 text-center">
          <p className="text-slate-400 text-sm font-medium">
            {t('noAccountYet')}{' '}
            <button onClick={onNavigateToSignup} className="text-[#2ECC71] font-black hover:underline">{t('createProfile')}</button>
          </p>
        </div>
      </div>
    </div>
