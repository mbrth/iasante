...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, setView, onLogout }) => {
  const { t } = useLanguage();
  const navItems = [
    { id: 'dashboard', label: t('overview'), icon: 'M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z' },
    { id: 'meals', label: t('journal'), icon: 'M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z' },
    { id: 'plan', label: t('protocol'), icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2' },
    { id: 'assistant', label: t('assistant'), icon: 'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z' },
    ...
    <aside className="w-20 md:w-72 bg-brand-secondary flex flex-col items-center py-10 transition-all duration-500">
      <div className="flex items-center gap-3 mb-16 px-6">
        ...
        <div className="hidden md:block">
          ...
          <span className="text-brand-primary text-[10px] font-black uppercase tracking-[0.2em]">{t('bioIntelligence')}</span>
        </div>
      </div>
      ...
      <div className="mt-auto px-4 w-full space-y-4">
         ...
         <div className="hidden md:block p-5 bg-white/5 rounded-3xl border border-white/10">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-[10px] font-black text-brand-primary uppercase tracking-widest">{t('medicalLink')}</p>
            </div>
            <p className="text-xs font-bold text-slate-300">{t('liveSyncEnabled')}</p>
         </div>
         <button 
           onClick={onLogout}
         ...
         >
            ...
            <span className="hidden md:block ml-4 text-sm">{t('signOut')}</span>
         </button>
      </div>
    </aside>
  );
