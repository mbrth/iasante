...
export const generateMedicalReport = (
  ...
  cursorY += (splitFeedback.length * 5) + 10;

  // --- NEW: ML INSIGHTS (STABILITY & NUTRITION) ---
  if (mlInsights) {
    doc.setTextColor(PRIMARY_COLOR[0], PRIMARY_COLOR[1], PRIMARY_COLOR[2]);
    doc.text("ANALYSE COMPLÉMENTAIRE (ML)", 20, cursorY);
    cursorY += 10;

    const mlData = [
      ["Stabilité Métabolique", `${mlInsights.metabolicStability}%`, "Score Nutrition", `${mlInsights.nutritionAnalysis?.score || '--'}/100`],
      ["Risque Cardiovasculaire", `${mlInsights.cvRisk}%`, "Tendance Poids", mlInsights.weightPrediction?.trend === 'decreasing' ? 'Baisse' : 'Stable'],
      ["Prédiction (Poids)", `${mlInsights.weightPrediction?.predicted?.toFixed(1) || '--'} kg`, "", ""],
    ];
    ...
    // Recommendations
    if (mlInsights.deterministicRisk?.recommendations?.length > 0) {
      doc.setTextColor(PRIMARY_COLOR[0], PRIMARY_COLOR[1], PRIMARY_COLOR[2]);
      doc.text("RECOMMANDATIONS CLINIQUES", 20, cursorY);
      cursorY += 8;
      ...
      mlInsights.deterministicRisk.recommendations.forEach((rec: string) => {
        const splitRec = doc.splitTextToSize(`• ${rec}`, pageWidth - 40);
        doc.text(splitRec, 20, cursorY);
        cursorY += (splitRec.length * 5);
      });
      cursorY += 10;
    }
  }
